...
public class MainActivity extends AppCompatActivity {
    ...
    public void addToWaitlist(View view) {
        ...
        String strGuestName = mNewGuestNameEditText.getText().toString().trim();
        String strPartySize = mNewPartySizeEditText.getText().toString().trim();
        if (strGuestName.isEmpty() || strPartySize.isEmpty()) {
            Log.e(LOG_TAG, "Empty guest namr or party size!");
            return;
        }
        ...
        try {
            partySize = Integer.parseInt(strPartySize);
        } catch (NumberFormatException e) {
            Log.v(LOG_TAG, "Invalid party size: " + strPartySize + ", use 1 instead.");
        }

        // TODO (14) call addNewGuest with the guest name and party size
        addNewGuest(strGuestName, partySize);
